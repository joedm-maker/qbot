...
2. Slack signature verified via `verify.mjs` (headers must be lowercased — API Gateway preserves original casing)
3. `event_callback` → `game-flow.mjs`
4. `block_actions` → routed by `action_id` to game-flow / score-entry / leaderboard
5. `view_submission` → routed by `callback_id`
6. `qbim_admin_*` → `score-entry.mjs`
...
- **Word parsing:** `+`, space, comma all equivalent separators. Hyphens assert explicit card boundaries (no alternatives offered). QU/TH/CL have distinct scores and prompt via radio; IN/ER are auto-resolved by hand fit.
