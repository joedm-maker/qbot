...
Slack bot for tracking QBIM/Quickler/Hot Swap/Qlander/Gauntlet (Quiddler variants) scores at Design Master, plus the AutoQ solo mode. Node.js 22 ESM on AWS Lambda, API Gateway, DynamoDB, deployed with AWS SAM. Companion React dashboard lives in the sibling `qbim-stats-dashboard/` repo and consumes the `/stats/*` endpoints from this Lambda.
...
- **Dealing vs. scoring cap are two different numbers.** Every hand *deals* `hand + 3` cards — **all** game types, physical and digital (`dealSizeForHand(gameType, hand, mulligans)` = `hand + 3 - mulligans` is the source of truth). But you **always keep at least 3 of them as discards** (can keep more), so the **max scorable cards in a word is `hand - mulligans`** (`Math.max(2, hand - mulligans)`) — each mulligan re-deals one fewer card, scaling the cap down with it. The scoring cap lives at the `maxCards` call sites in `score-entry.mjs` (submit + admin edit), `web-game.mjs`, and `autoq.mjs` — all use `hand - mulligans`, **not** `dealSizeForHand`. (Historical note: the QBIM/Quickler paths mistakenly used `dealSizeForHand` as the cap, so over-length words slipped through in physical games until this was corrected.)
